...
interface HeaderProps {
  greeting: string;
  statusLabel: string;   // напр. "🔥 Практик · x1.1"
  stars: number;
  streak: number;        // дней ХИТ подряд
  starsBumpKey?: number;
}

export function Header({ greeting, statusLabel, stars, streak, starsBumpKey = 0 }: HeaderProps) {
  return (
    <header className="safe-top px-5 pt-3 pb-2 flex items-center justify-between gap-3">
      <div className="min-w-0">
        ...
        <h1 className="text-[15px] font-semibold leading-tight truncate">{statusLabel}</h1>
      </div>
      ...
      <div className="flex items-center gap-2 shrink-0">
        ...
        <div
          ...
          aria-label="ХИТ подряд"
        >
          <Flame className="h-3.5 w-3.5" strokeWidth={2.5} />
          <span className="text-[13px] font-semibold tabular-nums">{streak}</span>
          <span className="text-[10px] font-bold opacity-90 ml-0.5">ХИТ</span>
        </div>
      </div>
    </header>
  );
