...
function StepScreen() {
  ...
  const { streakDays } = useEffectiveProgress("wishes");
...
🏆 Награда: +1 очко ежедневно за выполнение.`}
            ...
            {howTab === "video" && (
              <HowVideoCards
                first={{
                  title: "🎬 Почему один шаг в день меняет всё",
                  duration: "5:20",
                  caption:
                    "Как система ежедневных шагов приводит к большим результатам и почему именно «1 шаг» так важен.",
                }}
                second={{
                  title: "🎯 Как правильно работать с задачами",
                  duration: "4:40",
                  caption:
                    "Как разбивать цели на задачи, как выбирать шаг дня и как не сорваться, когда нет настроения.",
                }}
              />
            )}
          </div>
