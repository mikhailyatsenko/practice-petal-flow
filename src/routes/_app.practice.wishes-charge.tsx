...
export const Route = createFileRoute("/_app/practice/wishes-charge")({
  head: () => ({
    meta: [
      { title: "Вдохновение желаниями — Клуб «Моя жизнь»" },
      {
        name: "description",
        content:
          "Вдохнови свои желания эмоцией: лайкни каждое и одно вдохнови на 100%.",
      },
    ],
  }),
...
function ChargeScreen() {
  ...
  // Реальные данные о вдохновении желаний/целей из общего хранилища
  ...
  // Автоматически засчитываем вдохновение, как только оба условия выполнены.
  useEffect(() => {
  ...
  return (
    <div className="px-4 pt-2 pb-8">
          ...
          <div className="min-w-0">
            ...
            >
              {doneToday
                ...
                : "Вдохнови все желания, одно — на 100%"}
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="mt-3">
        ...
        <div className="space-y-2">
          ...
          <ChecklistItem
            ...
            text="Одно желание или цель вдохновить на 100%"
      ...
      {/* 6. Кнопка действия / статус выполнения.
          ...
          и кнопка превращается в серый бейдж «✅ Выполнено сегодня». */}
      <section className="mt-4">
        <button
          onClick={doneToday ? undefined : goToWishes}
          disabled={doneToday}
        ...
        >
          {doneToday
            ? "✅ Выполнено сегодня"
            : "❤️ Перейти к желаниям и вдохновиться"}
        </button>
        {!doneToday && (
          <p
            ...
            style={{ color: "#9ca3af" }}
          >
            Поставь лайки всем желаниям и целям — одно вдохновить на 100%
          </p>
        )}
      </section>
          ...
          <div className="mt-3 animate-fade-up">
            ...
            {howTab === "text" && (
              <div className="space-y-3">
                <HowCard
                  title="⚡ Что такое Вдохновение желаниями"
                  text={`Вдохновить желание — это сделать его живым внутри себя. Пока желание просто написано — оно мёртвое, это просто слова. Когда ты смотришь на него и чувствуешь эмоцию — оно оживает, становится вдохновлённым энергией.

Представь аккумулятор. Ненаполненный аккумулятор не работает. Желание без эмоции — то же самое. Ты вдохновляешь его своей верой и ощущением что это уже твоё.

Вдохновлённое желание — это желание к которому у тебя есть живая эмоциональная связь. Ты не просто знаешь что оно есть — ты его чувствуешь. Оно тебя притягивает, вдохновляет, заряжает энергией. Невдохновлённое — висит в списке мёртвым грузом. Ты про него забываешь. Оно не работает.

Оба смысла работают одновременно: ты вдохновляешь желание — вливаешь в него свою эмоцию и веру. И ты вдохновляешься от желания — получаешь энергию и вдохновение.`}
                />
                <HowCard
                  title="🎯 Как выполнять?"
                  text={`Перейди в раздел «Желания» и нажимай на сердечко рядом с каждым желанием и целью — каждое нажатие добавляет 20% вдохновения. Пройди по всем.

Одно желание или цель нажми 5 раз до 100% — это твоё глубокое вдохновение дня. В этот момент по-настоящему представь что это уже произошло. Не процесс достижения — а результат. Ты уже там. Почувствуй радость, гордость, свободу.`}
                >
                  <div
            ...
            {howTab === "video" && (
              <HowVideoCards
                first={{
                  ...
                  duration: "6:30",
                  caption:
                    "Как правильно вдохновляться желаниями, как входить в нужное эмоциональное состояние и почему это работает.",
                }}
                second={{
                  title: "🔥 Техника ежедневной практики",
                  duration: "5:00",
                  caption:
                    "Пошаговый разбор: как вдохновляться желаниями каждый день за 5 минут и удерживать состояние в течение дня.",
                }}
              />
            )}
          </div>
