...
const HOW_IT_WORKS_TEXT: Record<TabId, string> = {
  wants: `✨ Хотелки

Каждое большое желание когда-то начиналось с одной простой мысли — "а хочу я вот это". Этот раздел — место где эта мысль получает своё первое воплощение. Ты фиксируешь её, даёшь ей существование. Это начальная стадия любого желания.
...
🎯 Как выполнять? Когда ты открываешь раздел — все твои хотелки сразу появляются перед тобой. Чтобы добавить новую — нажми "Добавить хотелку" внизу и напиши своё желание текстом — как чувствуешь, своими словами. Без правил и шаблонов. Просто то, чего хочешь. Как только написал — хотелка сохранена ✅

💎 Что ты получаешь? Зафиксированное желание — это уже не просто мысль. Это намерение, которое существует. И чем больше таких намерений ты фиксируешь — тем яснее становится картина того, чего ты на самом деле хочешь от жизни.`,
  wishes: `🌟 Желания

Хотелка стала чем-то большим? Ты чувствуешь что это не просто мимолётная мысль, а то чего ты действительно хочешь? Тогда пора переводить её на следующий уровень — в желание.

Желание отличается от хотелки тремя вещами:

Ты знаешь зачем оно тебе — у тебя есть ответ на вопрос "почему это важно для меня?". Не просто "хочу и всё", а глубокое понимание того, что это желание даст тебе, как изменит твою жизнь, что ты почувствуешь, когда оно исполнится.

Ты видишь его образ — в твоей голове есть конкретная картинка. Не размытое ощущение, а живой образ: как это выглядит, где ты находишься, что тебя окружает. Чем конкретнее образ — тем сильнее притяжение к желанию.

Оно живёт в тебе достаточно сильно чтобы ты потратил время на его оформление.
...
🎯 Как выполнять? Когда ты открываешь этот раздел — все твои желания сразу появляются перед тобой. Перечитывай их, живи ими, чувствуй их. Чтобы добавить новое желание — нажми "Добавить желание" внизу. Бот проведёт тебя по трём шагам: Шаг 1. Перенеси желание из хотелок или создай новое — напиши его текстом. Шаг 2. Напиши причины — зачем тебе это желание? Что оно даст тебе? Чем важнее и глубже причины — тем сильнее желание притягивает тебя к себе. Шаг 3. Добавь картинку — образ, который отражает твоё желание. Визуальный образ многократно усиливает притяжение к желанию. Когда все три шага выполнены — желание сохранено ✅

Нажав на любое желание — ты попадёшь внутрь и увидишь его картинку и причины. Там же доступны кнопки: ✏️ Редактировать желание — обновить название, причины или картинку. ✅ Желание воплощено! — когда желание исполнилось, нажми эту кнопку. Оно перейдёт в финальный статус — Воплощённые желания. ❌ Удалить желание — удалить желание полностью.

💎 Что ты получаешь? Желание с причинами и образом — это уже не размытая фантазия. Это живой ориентир, который каждый раз когда ты его видишь — напоминает тебе, кем ты хочешь стать и куда идёшь. Чем чаще ты смотришь на свои желания — тем сильнее они притягивают тебя к себе.`,
  goals: `🎯 Цели

Желание стало настолько живым и важным, что ты готов идти к нему? Тогда пора переводить его в цель.

Цель отличается от желания одним — у неё есть конкретика:

Дата — ты знаешь до какого числа хочешь это реализовать. Дедлайн превращает "когда-нибудь" в "к конкретному сроку".

Критерий достижения — ты точно знаешь как поймёшь что цель воплощена. Не "хочу быть здоровым", а "пробежал 21 км".

План — у тебя есть конкретные шаги, которые ведут к цели. Не общие слова, а действия с датами.

Исследования показывают, что люди, которые записывают свои цели с конкретными датами и планом — достигают их в разы чаще тех, кто просто держит желание в голове. Это не теория. Это механика. И именно эту механику запускает этот раздел.

Когда у желания появляется дедлайн и план — происходит кое-что важное. Мозг перестаёт воспринимать его как "когда-нибудь" и начинает искать пути прямо сейчас. Ты начинаешь замечать возможности, людей, ресурсы, которые раньше проходили мимо. Цель активирует мозг, и он начинает работать на тебя.

🎯 Как выполнять? Когда ты открываешь этот раздел — все твои цели сразу появляются перед тобой. Чтобы добавить новую цель — нажми "Добавить цель" внизу. Бот проведёт тебя по четырём шагам: Шаг 1. Выбери желание, которое переводишь в цель. Шаг 2. Напиши конкретный критерий достижения — как именно ты поймёшь что цель воплощена? Шаг 3. Напиши дату, до которой хочешь реализовать цель в формате день.месяц.год. Дедлайн превращает намерение в обязательство. Шаг 4. Напиши общий план реализации. Разбей путь к цели на конкретные шаги. Когда все четыре шага выполнены — цель сохранена ✅

Нажав на любую цель — ты попадёшь внутрь и увидишь картинку, критерий достижения, план реализации и все задачи по этой цели. Там же доступны следующие действия: ➕ Добавить задачу — добавь разовую задачу к этой цели. 🧠 Мозговой штурм — 30 мощных вопросов, которые помогут тебе глубже прояснить как достичь цели. ✏️ Редактировать цель — изменить название, критерий достижения, дату или план. ✅ Желание воплощено! — когда цель достигнута, нажми эту кнопку. Цель перейдёт в Воплощённые желания. ❌ Убрать цель — удалить цель полностью.

💎 Что ты получаешь? Цель с критерием, датой и планом — это уже не мечта. Это проект. У него есть начало, конец и маршрут. Каждый раз когда ты открываешь этот раздел и видишь свои цели — ты видишь не просто слова, а конкретный курс, которому ты следуешь, и именно это запускает движение.`,
  tasks: `📋 Задачи

Цель — это направление. Задачи — это движение. Можно годами смотреть на цель и ждать подходящего момента. А можно каждый день делать один конкретный шаг. Именно из таких шагов складываются результаты, которые меняют жизнь. Этот раздел создан для того, чтобы ты всегда знал что делать прямо сейчас.

Большая цель пугает только пока ты смотришь на неё целиком. Стоит разбить её на конкретные задачи — и она перестаёт быть страшной. Любую цель можно достичь. Даже самую амбициозную. Просто нужно есть слона по кусочкам. Каждая задача это один кусочек. И однажды ты оглядываешься назад и понимаешь — слон съеден.

🎯 Как добавить задачу? Нажми "Добавить задачу". Бот проведёт тебя по шагам: Шаг 1. Выбери цель, к которой относится задача. Шаг 2. Напиши название задачи — конкретно и ясно. Не "поработать над проектом", а "написать первую главу". Шаг 3. Укажи, сколько дней тебе нужно на решение задачи — выбери из списка или напиши дату завершения вручную. Шаг 4. Укажи, сколько времени потребуется, если непрерывно заниматься задачей — выбери из списка. Шаг 5. Выбери чувство, которое вызывает у тебя эта задача — от максимально негативного до максимально позитивного. Задача создана ✅

✅ Выполнив хотя бы одну задачу в день — ты автоматически закрываешь пятую ежедневную привычку "Задачи к целям".

🎯 Как работать с задачами? В разделе задач ты видишь все задачи по всем целям. Используй фильтры чтобы быстро находить нужное: 🟧 На день — задачи назначенные на сегодня 🟦 На неделю — задачи на эту неделю 🟪 На месяц — задачи на этот месяц ⬜ Открытые — все незавершённые задачи 🟥 Главные задачи — ключевые задачи, решив которые ты сделаешь большой прогресс к реализации цели 🏁 Решённые задачи — здесь по каждой цели видно сколько задач уже выполнено

Нажав на любую задачу — ты попадёшь внутрь. Там доступны действия: ▶️ Начать работу — запускает таймер. Бот фиксирует сколько времени ты реально потратил на задачу. ✏️ Редактировать задачу — изменить название, дату или другие параметры. ✅ Задача сделана! — отметь когда задача выполнена. Она уйдёт из активных. ❌ Удалить задачу — удалить задачу полностью. 🎯 Перейти к цели — быстрый переход к цели к которой относится задача.

💎 Что ты получаешь? Задачи превращают большую цель в управляемый процесс. Вместо того, чтобы смотреть на цель и не знать с чего начать — ты просто берёшь следующую задачу и делаешь. Шаг за шагом. День за днём. И цель перестаёт казаться далёкой.`,
  done: `🏆 Воплощённые желания

Это не просто раздел. Это твоя история побед. Каждое желание и хотелка которые ты довёл до воплощения — живут здесь.

Многие люди не замечают своих побед — они достигают одного и сразу бегут к следующему. Этот раздел создан, чтобы ты останавливался и видел — сколько всего уже воплощено. Это мощный источник веры в себя и в свои будущие желания.

💎 Как пополняется этот раздел? Автоматически — когда ты нажимаешь "Желание воплощено!" внутри любого желания или цели. Воплощённое желание сразу появляется здесь и хранится навсегда.

💎 Как использовать этот раздел? Возвращайся сюда, когда чувствуешь сомнение или усталость. Листай свои воплощённые желания. Читай причины, которые когда-то двигали тебя вперёд. Смотри на картинки. Это лучшее напоминание о том, на что ты способен.`,
};
